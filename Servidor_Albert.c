	...
	for(;;){
		...
		{// Ahora recibimos su nombre, que dejamos en buff
			...
			else if (codigo == 4)
			{
				...
				int  respuesta_int = LogIn(conn, resultado, row, err, nom, contrasenya);
				if(respuesta_int == 1)
					sprintf(buff2, "1");
				else
					sprintf(buff2, "0");
...
void Consulta_DuracioMin(MYSQL *conn, MYSQL_RES *resultado, MYSQL_ROW row, int err,char buff2[512])
{
	...
	err = mysql_query (conn, "SELECT PARTIDA.ID FROM (PARTIDA)WHERE PARTIDA.DURACIO = (SELECT MIN(PARTIDA.DURACIO) FROM PARTIDA)");
	printf("error= %d\n",err);
	if (err!=0) {
...
{
	printf("entrar dins retornar contasenya\n");
	char consulta [200];
	//Creamos una conexion al servidor MYSQL 
...
void Consulta_MaxParticipacions(MYSQL *conn, MYSQL_RES *resultado, MYSQL_ROW row, int err, char buff2[512])
{
	...
	if (row == NULL)
		...
		while (row !=NULL) {
			// la columna 0 contiene el nom amb mï¿©s participacions
			sprintf (buff2,"%s%s,",buff2, row[0]);
			// obtenemos la siguiente fila ya que dos o mas partidas 
			//pueden haber durado lo mismo
			row = mysql_fetch_row (resultado);
